...
class BERT(nn.Module):
    ...
    def forward(self, text_list):
        ...
        # TODO: figure out how to pad to max length <max_length
        if self.mlm:
...
class BertEmbedding(nn.Module):
    def __init__(self, config):
        ...
        self.update_bert_config()
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        if self.config.MODEL.LANGUAGE_BACKBONE.PRETRAINED:
            self.bert_model = BertModel.from_pretrained(
                'bert-base-uncased', config=self.bert_config).embeddings   
        else:
            self.bert_model = BertModel(config=self.bert_config).embeddings   
        self.freeze()
        self.out_channels = self.bert_config.hidden_size
        head_config = self.config.MODEL.MMSS_HEAD.TRANSFORMER
        self.mlm = head_config.MASKED_LANGUAGE_MODELING
        ...
        self.mlm_prob_noise = head_config.MASKED_LANGUAGE_MODELING_PROB_NOISE
        self.mlm_during_validation = head_config.MASKED_LANGUAGE_MODELING_VALIDATION
        self.embeddings = self.bert_model.word_embeddings.weight
    ...
    def forward(self, text_list):
        ...
        if self.config.MODEL.LANGUAGE_BACKBONE.ADD_POSITION_EMBEDDING:
            bertemb_output = self.bert_model(
                input_ids=tokenized_batch['input_ids'],
                token_type_ids=tokenized_batch['token_type_ids'],
                )
            
            tokenized_batch['encoded_tokens'] = bertemb_output
        else:
            # By using this I am not using the positional embeddings
            tokenized_batch['encoded_tokens'] = self.embeddings[tokenized_batch['input_ids']]
        tokenized_batch['input_embeddings'] = self.embeddings[tokenized_batch['input_ids']]
    ...
    def freeze(self):
        if self.config.MODEL.LANGUAGE_BACKBONE.FREEZE:
            ...
            for name, param in self.named_parameters():
                if param.requires_grad and name != "bert_model.word_embeddings.weight":
