...
MMSS_HEADS_REGISTRY = Registry("MMSS_HEADS")

__all__ = ["GroundingHead"]
...
def choose_one(tensor):
    return wrap_nd_batch(
        lambda x: torch.multinomial(x, num_samples=1).squeeze(-1), tensor
    )
...
def remove_diag(square_matrix, dim):
    ...
    """
    assert (
        len(square_matrix.shape) == 2
        and square_matrix.shape[0] == square_matrix.shape[1]
        and dim in [0, 1]
    )
...
class GroundingHead(LoggedModule):
    def __init__(self, config, v_dim, l_dim, *args, **kwargs):
        ...
        # weather to return the pair scores for distillation
        self.grounding_text_input = self.config.TEXT_INPUT

    def forward(self, input_image, input_caption):
        caption_emb = input_caption[self.grounding_text_input]
        caption_mask = input_caption["attention_mask"] * (
            1 - input_caption["special_tokens_mask"]
        )
        ...
        self.log("region_features", region_features)
        self.log("region_mask", region_mask)
        ...
        if self.loss_type == "cross_entropy" or self.loss_type == "triplet":
            ...
            image_emb = (
                ...
                .reshape(batch_size ** 2, self.l_dim, max_num_regions)
            )
            caption_emb = (
                caption_emb[:, None, :, :]
            ...
            num_regions = (
                num_regions[None, :].repeat(batch_size, 1).reshape(batch_size ** 2)
            )
            num_words = (
                num_words[:, None].repeat(1, batch_size).reshape(batch_size ** 2)
            )
        ...
        if self.alignment == "softmax":
            ...
            if self.align_regions:
                ...
                attention_r2w = (
                    F.one_hot(idx, max_num_words).to(torch.float32).permute(0, 2, 1)
                )
            ...
            if self.align_words:
                ...
                attention_w2r = (
                    F.one_hot(idx, max_num_regions).to(torch.float32).sum(dim=2)
                )
                idx = choose_one(attention_w2r)
                attention_w2r = F.one_hot(idx, max_num_regions).to(torch.float32)
            if self.align_regions:
                ...
                attention_r2w = (
                    F.one_hot(idx, max_num_words).to(torch.float32).sum(dim=1)
                )
                idx = choose_one(attention_r2w)
                attention_r2w = (
                    F.one_hot(idx, max_num_words).to(torch.float32).permute(0, 2, 1)
        ...
        if self.global_metric == "reconstruction_mse":
            if self.align_words:
                ...
                global_dist_w2r = (global_dist_w2r * caption_mask).sum(
                    dim=1
            ...
            if self.align_regions:
                ...
                global_dist_r2w = (global_dist_r2w * region_mask).sum(
                    dim=1
            ...
            if self.align_words:
                ...
                global_dist_w2r = (attention_w2r * local_distance).sum(dim=2).sum(
                    dim=1
                ) / torch.max(num_words, other=torch.ones_like(num_words))
            if self.align_regions:
                attention_r2w = attention_r2w * region_mask[:, None, :]
                global_dist_r2w = (attention_r2w * local_distance).sum(dim=2).sum(
                    dim=1
                ) / torch.max(num_regions, other=torch.ones_like(num_regions))
        else:
            raise NotImplementedError
        ...
        if self.loss_type == "matching":
            if self.local_metric == "dot":
                raise Exception(
                    "Matching loss is not defined for dot product\
                                 because dot product is unbounded"
                )
            if self.align_words:
                losses["ImgCap_Match_loss (Align Words)"] = global_dist_w2r.mean()
            if self.align_regions:
                losses["ImgCap_Match_loss (Align Regions)"] = global_dist_r2w.mean()
            ...
            if self.align_words:
                ...
                losses["CE_loss (Align Words, Choose Caption)"] = torch.diag(
                    -pw_logits_c_cap_w2r
                ).mean()
                losses["CE_loss (Align Words, Choose Image)"] = torch.diag(
                    -pw_logits_c_img_w2r
                ).mean()
            if self.align_regions:
                ...
                losses["CE_loss (Align Regions, Choose Caption)"] = torch.diag(
                    -pw_logits_c_cap_r2w
                ).mean()
                losses["CE_loss (Align Regions, Choose Image)"] = torch.diag(
                    -pw_logits_c_img_r2w
                ).mean()
        ...
        if self.loss_type == "cross_entropy" or self.loss_type == "triplet":
            if self.align_words:
                other_info["Batch Accuracy (Align Words, Choose Caption)"] = torch.mean(
                    (
                        pw_cost_w2r.argmin(dim=0) == torch.arange(batch_size).to("cuda")
                    ).to(torch.float32)
                )
                other_info["Batch Accuracy (Align Words, Choose Image)"] = torch.mean(
                    (
                        pw_cost_w2r.argmin(dim=1) == torch.arange(batch_size).to("cuda")
                    ).to(torch.float32)
                )
            if self.align_regions:
                other_info[
                    "Batch Accuracy (Align Regions, Choose Caption)"
                ] = torch.mean(
                    (
                        pw_cost_r2w.argmin(dim=0) == torch.arange(batch_size).to("cuda")
                    ).to(torch.float32)
                )
                other_info["Batch Accuracy (Align Regions, Choose Image)"] = torch.mean(
                    (
                        pw_cost_r2w.argmin(dim=1) == torch.arange(batch_size).to("cuda")
                    ).to(torch.float32)
                )
        ...
        if self.return_dist:
            distributions = {"w2r": pw_cost_w2r, "r2w": pw_cost_r2w}
            return other_info, losses, distributions

        return other_info, losses


def build_grounding_head(name, cfg, v_dim, l_dim, *args, **kwargs):
    return MMSS_HEADS_REGISTRY.get(name)(cfg, v_dim, l_dim)
